...
tau = 29/np.sqrt(2.*np.log(2))*1e-15     # Laser duration   40fs FWHM
ChirpStretching = 1.001     #Pulse lenghening due to chirp
ChirpSign = -1     #Positive (1)  or negative (-1) chirp
ctau=c*tau        #Pulse length
ctau_stretched= ctau*ChirpStretching

z0 = 0.e-6     # Laser centroid
zfoc = 2000.e-6     # Focal position
...
n_e = 0.9e18*1e6      # The density in the labframe (electrons.meters^-3)
...
zmax = 2.2*ctau_stretched     # Length of the box along z (meters)
zmin = -2*ctau_stretched-1.5*lambda_p
rmax = 5*w0   # Length of the box along r (meters)
...
# The density profile
w_matched = 400000000000000000.e-6
ramp_up = 1000.e-6
plateau = 20e3*1.e-6
ramp_down = ramp_up
...
if __name__ == '__main__':
    ...
    smoother = BinomialSmoother( {'z':4,'r':4}, compensator={'z':True
                                                             ,'r':True} )
    
    sim = Simulation( Nz, zmax, Nr, rmax, Nm, dt, zmin=zmin,
        n_order=n_order, use_cuda=use_cuda,
        boundaries={'z':'open', 'r':'reflective'},smoother=smoother)
    ...
    profile = GaussianLaser(a0=a0, waist=w0, tau=tau, z0=z0, lambda0=lambdad, zf=zfoc, phi2_chirp=GDD)
