...
def main():
    ...
    chosen_y_vals = generate_slices(DIM_MAPPING["y"], np.array(random.sample(y_values, 4)), DIM_MAPPING["x"])
    chosen_x_vals = generate_slices(DIM_MAPPING["x"], np.array(random.sample(x_values, 4)), DIM_MAPPING["y"])
    ...
    for i, j in np.ndindex(charge.shape[:-1]):
        match = proj.find_jobs(filter={DIM_MAPPING["y"]: y_values[i], DIM_MAPPING["x"]: x_values[j]})
    ...
    #
    spectra.coords[DIM_MAPPING["x"]].attrs["plot_label"] = r"$n_e$ ($10^{18}\,\mathrm{cm^{-3}}$)"
