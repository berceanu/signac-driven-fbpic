...
from fbpic.lpa_utils.bunch import add_particle_bunch_gaussian, add_particle_bunch_file
...
zmax = 3000.e-6    # Right end of the simulation box (meters)
zmin = -39000.e-6   # Left end of the simulation box (meters)
Nr = 250          # Number of gridpoints along r
rmax = 500.e-6    # Length of the box along r (meters)
...
p_zmax = 68000.e-6 # Position of the end of the plasma (meters)
p_rmax = 400.e-6  # Maximal radial position of the plasma (meters)
n_e = 5.e13*1.e6 # Density (electrons.meters^-3)
...
# The electron beam
L0 = 100.e-6      # Position at which the beam should be "unfreezed"
z0 = 200.e-6       # Initial centroid of the beam
sig_z = 500.e-6    # RMS size
sig_r = 20.e-6    # RMS size
n_emit = 1.e-6   # Normalized emittance in m.rad
gamma0 = 200.
sig_gamma = 0.5
Qtot = 200.e-12   # Charge in Coulomb
...
L_interact = 68000.e-6 # increase to simulate longer distance!
...
if __name__ == '__main__':
    ...
    elec = sim.add_new_species( q=-e, m=m_e, n=n_e,
        dens_func=dens_func_ext, p_zmin=p_zmin, p_zmax=p_zmax, p_rmax=p_rmax,
        p_nz=p_nz, p_nr=p_nr, p_nt=p_nt )

    #particles beam
    #beam = add_particle_bunch_gaussian( sim, q=-e, m=m_e,
    #    sig_r=sig_r, sig_z=sig_z, n_emit=n_emit, 
    #    gamma0=gamma0, sig_gamma=sig_gamma,
    #    n_physical_particles=Qtot/e, n_macroparticles=50000,
    #    zf=L0, tf=(L0-z0)/c, z_injection_plane=L0 )

    #particles beam from txt file
    beam = add_particle_bunch_file( sim, q=-e, m=m_e,
        filename='../exp_4deg.txt', n_physical_particles = Qtot/e,
        z_off=0.0 , z_injection_plane=L0 )                         
