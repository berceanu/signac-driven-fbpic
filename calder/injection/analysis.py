...
n_c = 1.75e+27 # critical electron density in electrons x m^{-3}
...
def field_snapshot(tseries, iteration, field, norm_factor, coord=None, m='all', theta=0.,
                    chop = [0., 0., 0., 0.], **kwargs):
    F, info = tseries.get_field(field=field, coord=coord, iteration=iteration,
                            m=m, theta=theta)
                            #
    F *= norm_factor
    plot = plotz.Plot2D(
                    F, info.z * 1e6, info.r * 1e6,
                    xlabel=r'${} \;(\mu m)$'.format(info.axes[1]),
                    ylabel=r'${} \;(\mu m)$'.format(info.axes[0]),
                    extent=(info.zmin * 1e6 + chop[0], info.zmax * 1e6 + chop[1],
                             info.rmin * 1e6 + chop[2], info.rmax * 1e6 + chop[3]),
                    cbar=True,
...
def particle_histogram(tseries, iteration, Emin=1., Emax=300., nbins=100,
                         mc2=0.5109989462686102, q_e=1.6021766208e-19):
    dE = (Emax - Emin)/nbins
    E_bins = np.linspace( Emin, Emax, nbins+1 )
    ux, uy, uz, w = tseries.get_particle(
    ['ux', 'uy', 'uz', 'w'], iteration=iteration )
    E = mc2 * np.sqrt( 1 + ux**2 + uy**2 + uz**2 )  # Energy in MeV
    Q_bins, edges = np.histogram( E, E_bins, weights = q_e * 1e12 / dE * w )  # weights in pC/MeV
    return Q_bins.tolist(), edges.tolist()
                    

def apply_func(iteration):
    field_snapshot(tseries=ts_circ, iteration=iteration,
                    field='rho', norm_factor=unit_factor,
                    chop = [40, -20, 15, -15],
                    zlabel=r'$n/n_e$', vmin=0, vmax=3)
    time_fs = iteration * dt
    z0, a0, w0, ctau = pp.get_a0(ts_circ, iteration=iteration) # SI
    Q, edg = particle_histogram(tseries=ts_circ, iteration=iteration, Emin=1., Emax=350., nbins=349)
...
if __name__ == '__main__':
    ...
    plot = plotz.Plot1D(a0, z0, xlabel=r'$%s \;(\mu m)$'%'z', ylabel=r'$%s$'%'a_0', xlim=[0, 900], ylim=[0, 10],
                figsize=(10, 6), color='red')
    plot.canvas.print_figure('a0.png') # 
    #
    plot = plotz.Plot1D(w0, z0, xlabel=r'$%s \;(\mu m)$'%'z', ylabel=r'$%s \;(\mu m)$'%'w_0',
                figsize=(10, 6))
    plot.canvas.print_figure('w0.png') #
    #
    plot = plotz.Plot1D(ctau, z0, xlabel=r'$%s \;(\mu m)$'%'z', ylabel=r'$c \tau \;(\mu m)$')
    plot.canvas.print_figure('ctau.png') #


    z_min = ts_circ.iterations[0] * tstep_to_pos + 70 # mu
    z_max = ts_circ.iterations[-1] * tstep_to_pos + 70 # offset by 70 because left margin is at -70
    z_35100 = ts_circ.iterations[35100//100] * tstep_to_pos + 70
    #
    h_axis = np.linspace(z_min, z_max, ts_circ.iterations.size - 1)
    v_axis = np.linspace(1., 350., 349)
    #
    hist2d = plotz.Plot2D(Q.T, h_axis, v_axis,
                    xlabel=r'$%s \;(\mu m)$' % 'z', ylabel=r'E (MeV)', zlabel=r'dQ/dE (pC/MeV)',
                    # hslice_val=0,
                    vslice_val=z_35100,
                    # hslice_opts={'color': 'firebrick', 'lw' : 0.5, 'ls':'-'},
                    # vslice_opts={'color': 'blue', 'ls': '-'},
                    extent=(z_min, z_max, 1., 350.),
                    vmin=0, vmax=10,
                    # vmin=1e-3, vmax=1e2,
                    # norm = colors.LogNorm(),
                    )
    ...
    field_snapshot(tseries=ts_circ, iteration=35100, field='E', coord='x', norm_factor=1/pp.E0(),
                        chop = [40, -20, 15, -15], 
                        zlabel=r'$E_x / E_0$', vmin=-8, vmax=8,
                        hslice_val=0.)
